...
func main() {
	...
	for i := 0; i < 4; i++ {
		...
		gaborFilteredImage := image.NewGray(imoriImage.Bounds())
		max := math.Inf(-1)
		for y := 0; y < H; y++ {
			for x := 0; x < H; x++ {
				pixVal := 0.0
				...
				gaborFilteredImage.SetGray(x, y, color.Gray{uint8(pixVal)})
			}
		}

		for y := 0; y < H; y++ {
			for x := 0; x < H; x++ {
				pixVal := float64(gaborFilteredImage.GrayAt(x, y).Y) * 255 / max
				gaborFilteredImage.SetGray(x, y, color.Gray{uint8(pixVal)})
			}
		}

		gaborFilteredFile, err := os.Create(fmt.Sprintf("./answer_79_%d.jpg", θ))
		defer gaborFilteredFile.Close()
		if err != nil {
			log.Fatal(err)
		}
		jpeg.Encode(gaborFilteredFile, gaborFilteredImage, &jpeg.Options{100})
	}
