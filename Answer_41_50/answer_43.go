...
func main() {
	...
	// answer_42 ここから
	for y, angleRow := range tanArray {
		for x, angle := range angleRow {
			edge := edgeArray[y][x]
			if angle == 0 {
	...
	edgeImage = grayArray2grayImage(edgeArray)
	edgeImageFile, err = os.Create("./answer_42.jpg")
	defer edgeImageFile.Close()
	if err != nil {
		log.Fatal(err)
	}
	...
	cannyArray := make([][]float64, len(grayGaussianSobelhArray))
	for x := range cannyArray {
		cannyArray[x] = make([]float64, len(grayGaussianSobelhArray[0][:]))
	}
	copy(cannyArray, edgeArray)

	for y, row := range edgeArray {
		for x, edge := range row {
			if edge > HT {
				cannyArray[y][x] = 255.0
			} else if edge < LT {
				cannyArray[y][x] = 0.0
			} else if edge > LT && edge < HT {
				flag := false
				for j := y - 1; j <= y+1; j++ {
					for k := x - 1; k <= x+1; k++ {
						if edgeArray[j][k] > edge {
							flag = true
						}
					}
				}
	...
	cannyImage := grayArray2grayImage(cannyArray)
	cannyImageFile, err := os.Create("./answer_43.jpg")
	defer cannyImageFile.Close()
	if err != nil {
		log.Fatal(err)
	}
	jpeg.Encode(cannyImageFile, cannyImage, &jpeg.Options{100})
