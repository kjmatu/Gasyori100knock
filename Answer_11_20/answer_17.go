...
func main() {
	...
	// ラプラシアンフィルタ適用済画像保存先を作成
	laplacianImg := image.NewGray(grayimg.Bounds())
	filterSize := 3
	...
	for y := 0; y < grayimg.Bounds().Size().Y; y++ {
		for x := 0; x < grayimg.Bounds().Size().X; x++ {
			...
			laplacianImg.Set(x, y, filteredGray)
		}
	}

	laplacianFilterImgFile, err := os.Create("./answer_17.jpg")
	defer laplacianFilterImgFile.Close()
	if err != nil {
		log.Fatal(err)
	}
	jpeg.Encode(laplacianFilterImgFile, laplacianImg, &jpeg.Options{100})
