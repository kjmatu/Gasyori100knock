...
func main() {
	...
	// モルフォロジー処理(膨張)
	dialationImage := dialation(binImage)
	...
	for y := 0; y < binImage.Bounds().Size().Y; y++ {
		for x := 0; x < binImage.Bounds().Size().X; x++ {
			diaVal := dialationImage.GrayAt(x, y).Y
			eroVal := erosionImage.GrayAt(x, y).Y
			diff := diaVal - eroVal
			// fmt.Println("diaVal", diaVal, "eroVal", eroVal, "diff", diff)
			diffImage.Set(x, y, color.Gray{uint8(diff)})
		}
	}

	diffFile, err := os.Create("./answer_51.jpg")
