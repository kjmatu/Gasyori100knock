...
func main() {
	...
	// テンプレートマッチング SSD (Sum of Squared Difference)
	ssd := uint64(math.MaxUint64)
		...
		for x := 0; x < W; x++ {
			...
			for v := 0; v < h; v++ {
				for u := 0; u < w; u++ {
					...
					localSSD += (uint64((srcR - targetR) * (srcR - targetR)))
					localSSD += (uint64((srcG - targetG) * (srcG - targetG)))
					localSSD += (uint64((srcB - targetB) * (srcB - targetB)))
				}
			}
			if localSSD < ssd {
				ssd = localSSD
	...
	tamplateMatchFile, err := os.Create("./answer_54.jpg")
