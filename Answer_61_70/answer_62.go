...
func main() {
	...
	// 上記で作成した画像の0と1を反転する
	cnGrayInverseImage := image.NewGray(cnImage.Bounds())
	for y := 0; y < cnImage.Bounds().Size().Y; y++ {
		for x := 0; x < cnImage.Bounds().Size().X; x++ {
			cnGrayInverseImage.Set(x, y, color.Gray{1 - cnGrayImage.GrayAt(x, y).Y})
		}
	}
	...
	for y := 0; y < cnGrayImage.Bounds().Size().Y; y++ {
		for x := 0; x < cnGrayImage.Bounds().Size().X; x++ {
			...
			x1 := int(cnGrayInverseImage.GrayAt(rightIndex, y).Y)
			x2 := int(cnGrayInverseImage.GrayAt(rightIndex, upIndex).Y)
			x3 := int(cnGrayInverseImage.GrayAt(x, upIndex).Y)
			x4 := int(cnGrayInverseImage.GrayAt(leftIndex, upIndex).Y)
			x5 := int(cnGrayInverseImage.GrayAt(leftIndex, y).Y)
			x6 := int(cnGrayInverseImage.GrayAt(leftIndex, downIndex).Y)
			x7 := int(cnGrayInverseImage.GrayAt(x, downIndex).Y)
			x8 := int(cnGrayInverseImage.GrayAt(rightIndex, downIndex).Y)
	...
	cnColorFile, err := os.Create("./answer_62.png")
