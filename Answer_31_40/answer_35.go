...
func bandPassFileter(siftDftResult [][]complex128) [][]complex128 {
	H := len(siftDftResult)
	halfH := int(float64(H) / 2)
	...
	for y, rowArray := range siftDftResult {
		for x, dftValue := range rowArray {
			...
			// 指定された領域をカットする
			if distanceFromCenter >= float64(halfW)*0.1 && distanceFromCenter <= float64(halfW)*0.5 {
...
func main() {
	...
	// バンドパスフィルタをかける
	filteredSiftDftResult := bandPassFileter(siftDftResult)
	...
	bandPassImg := image.NewGray(jimg.Bounds())
	for y, rowArray := range filterdInvDftResult {
		for x, invDftVal := range rowArray {
			...
			bandPassImg.Set(x, y, invDftGray)
		}
	}

	bandPassFile, err := os.Create("./answer_35.jpg")
	defer bandPassFile.Close()
	if err != nil {
		log.Fatal(err)
	}
	jpeg.Encode(bandPassFile, bandPassImg, &jpeg.Options{100})
