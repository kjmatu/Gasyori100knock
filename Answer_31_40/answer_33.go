...
func lowPassFileter(siftDftResult [][]complex128) [][]complex128 {
	H := len(siftDftResult)
	halfH := int(float64(H) / 2)
	...
	for y, rowArray := range siftDftResult {
		for x, dftValue := range rowArray {
			...
			// 外周部にある高周波成分をカットする
			if distanceFromCenter > float64(halfW)/2 {
...
func main() {
	...
	// ローパスフィルタをかける
	filteredSiftDftResult := lowPassFileter(siftDftResult)
	...
	lowPassImg := image.NewGray(jimg.Bounds())
	for y, rowArray := range filterdInvDftResult {
		for x, invDftVal := range rowArray {
			...
			lowPassImg.Set(x, y, invDftGray)
		}
	}

	lowPassFile, err := os.Create("./answer_33.jpg")
	defer lowPassFile.Close()
	if err != nil {
		log.Fatal(err)
	}
	jpeg.Encode(lowPassFile, lowPassImg, &jpeg.Options{100})
