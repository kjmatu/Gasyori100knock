...
func main() {
	...
	for _, testFileName := range testFiles {
		...
		fmt.Println(filepath.Base(testFileName) + " is similar >> " + filepath.Base(trainFiles[diffMinIndex]))
		if pred[diffMinIndex] == 0 {
			fmt.Println("Pred >> " + strings.Split(filepath.Base(testFileName), "_")[1])
		} else if pred[diffMinIndex] == 1 {
			replacePred := map[string]string{"akahara": "madara", "madara": "akahara"}
			fmt.Println("Pred >> " + replacePred[strings.Split(filepath.Base(testFileName), "_")[1]])
		}
	}
